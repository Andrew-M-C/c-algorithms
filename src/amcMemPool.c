...
#ifdef _DEBUG_FLAG
#define _DEBUG(fmt, args...)    printf(__FILE__", Line %d: "fmt"\n", __LINE__, ##args)
#else
...
static struct AMCMemUnit *_memUnit_ByData(void *pData)
{
    return (AMCMemUnit_st*)(_ULONG_CVT(pData) - _ULONG_CVT(sizeof(AMCMemUnit_st)));
}


static void *_memData_ByUnit(struct AMCMemUnit *pUnit)
{
    return (void*)(_ULONG_CVT(pUnit) + sizeof(*pUnit));
}
...
int AMCMemPool_Free(void *pData)
{
    ...
    {
        pUnit = (AMCMemUnit_st*)(_ULONG_CVT(pData) - _ULONG_CVT(sizeof(AMCMemUnit_st)));
        return _memUnit_Free_WithLock(pUnit);
    }
}
...
/********/
#else
    struct AMCMemBlock *block;
    long totalUnitCount = 0;
    long freeUnitCount = 0;

    if (NULL == pool)
    {
        printf("Memory pool [NULL]\n");
        return;
    }
    
    printf("Memory pool [0x%08lx], ", _ULONG_CVT(pool));

    if (pool->unitSize >= (1 << 20))
    {
        printf("unit size %ld MB +%ld kB + %ld Bytes, ", 
            (pool->unitSize) >> 20 , 
            ((pool->unitSize) & ((1 << 20) - 1)) >> 10,
            ((pool->unitSize) & ((1 << 10) - 1)) >> 0);
    }
    else if (pool->unitSize >= (1 << 10))
    {
        printf("unit size %ld kB + %ld Bytes, ", 
            (pool->unitSize) >> 10 , 
            ((pool->unitSize) & ((1 << 10) - 1)) >> 0);
    }
    else
    ...
    for (block = pool->pBlocks; 
        ...
        block = block->pNext)
    {
        totalUnitCount += block->totalUnitCount;
        freeUnitCount += block->freeUnitCount;
    }
    _UNLOCK_POOL(pool);         /* -- UNLOCK -- */
